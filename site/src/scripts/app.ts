...
function renderCard(entry: Entry, column: Column): string {
  ...
  if (column === 'lny') {
    ...
    <p class="mt-2 text-sm italic text-gray-700">"${escapeHtml(entry.exact_phrase)}"</p>
    ...
    <a href="${escapeHtml(entry.source_url)}" target="_blank" rel="noopener noreferrer" class="mt-2 inline-block text-xs text-gray-900 hover:underline transition-colors duration-200">Source ↗</a>
  </div>`;
  }
